...
namespace GC_namespace
{
  ...
  static inline bool isZero( real_type x )
  {
  ...
  */
  ...
  static void lua_table_to_GC( lua_State * L, GenericContainer & gc );

  static void push_vec_element( lua_State * L, GenericContainer & gc )
  {
    // assegna il valore
    ...
    switch ( type )
    {
      ...
      case LUA_TNUMBER:
      {
        ...
        if ( gc.get_type() == GC_type::VEC_REAL || gc.get_type() == GC_type::MAT_REAL ) { gc.get_real_at( idx ) = val; }
        else if ( isInteger64( val ) ) { gc.get_long_at( idx ) = long_type( val ); }
        else
        {
      ...
      break;
      case LUA_TSTRING: gc.get_string_at( idx ) = lua_tostring( L, -1 ); break;
      case LUA_TTABLE: lua_table_to_GC( L, gc[idx] ); break;
    }
  }

  // -----------------------------------------------------------------------------

  static void push_hash_element( lua_State * L, GenericContainer & gc )
  {
    // assegna il valore
    ...
    switch ( type )
    {
      case LUA_TBOOLEAN: gc[key].set_bool( lua_toboolean( L, -1 ) ? true : false ); break;
      case LUA_TNUMBER:
      {
        real_type val = lua_tonumber( L, -1 );
        if ( isInteger64( val ) )
          gc[key].set_long( long_type( val ) );
        else
          gc[key].set_real( val );
      }
      break;
      case LUA_TSTRING: gc[key].set_string( lua_tostring( L, -1 ) ); break;
      case LUA_TTABLE: lua_table_to_GC( L, gc[key] ); break;
    }
  }
  ...
  static void lua_table_to_GC( lua_State * L, GenericContainer & gc )
  {
    ...
    while ( lua_next( L, -2 ) != 0 )
    {
      switch ( lua_type( L, -2 ) )
      {
        case LUA_TNUMBER: push_vec_element( L, gc ); break;
        case LUA_TSTRING: push_hash_element( L, gc ); break;
      }
      lua_pop( L, 1 );  // removes `value'; keeps `key' for next iteration
    }
  }

  static void lua_to_GC( lua_State * L, GenericContainer & gc )
  {
    gc.clear();
    switch ( lua_type( L, -1 ) )
    {
      case LUA_TBOOLEAN: gc.set_bool( lua_toboolean( L, -1 ) ? true : false ); break;
      case LUA_TNUMBER:
      {
        real_type val = lua_tonumber( L, -1 );
        if ( isInteger64( val ) )
          gc.set_long( long_type( val ) );
        else
          gc.set_real( val );
      }
      break;
      case LUA_TSTRING: gc.set_string( lua_tostring( L, -1 ) ); break;
      case LUA_TTABLE:
  ...
  */
  ...
  static void GC_to_lua( lua_State * L, GenericContainer const & gc )
  {
    // inizializzazione
    switch ( gc.get_type() )
    {
      case GC_type::NOTYPE: lua_pushnil( L ); break;
      case GC_type::POINTER: lua_pushnil( L ); break;
      case GC_type::BOOL: lua_pushboolean( L, gc.get_bool() ? 1 : 0 ); break;
      case GC_type::INTEGER: lua_pushnumber( L, gc.get_int() ); break;
      case GC_type::LONG: lua_pushnumber( L, lua_Number( gc.get_long() ) ); break;
      case GC_type::REAL: lua_pushnumber( L, gc.get_real() ); break;
      case GC_type::STRING: lua_pushstring( L, gc.get_string().data() ); break;
      case GC_type::VEC_POINTER: lua_pushnil( L ); break;
  ...
  */
  ...
  void LuaInterpreter::call( GenericContainer const & fun_io, GenericContainer & res )
  {
    lua_State *& L = *( reinterpret_cast<lua_State **>( &void_L ) );
